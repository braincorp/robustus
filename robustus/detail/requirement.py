...
def generate_dependency_list(visited_sites, selected_requirements=None):
    '''
    Generate list of all packages that were included, and
    where they were included from, specified by version.

    :param visited_sites: <Dict<list>> visited_sites generated by expand_requirements_expand_requirements_specifiers()
    :param selected_requirements: set of requirements that robustus will install
    ...
    marked_requirements = {}
    # Convert selected_requirements into dict
    ...
    for key in sorted_packages:
        ...
        for version in package_version_from[key].keys():
            ...
            if key in marked_requirements and marked_requirements[key] == version:
                marked_for_install = "[to be installed]"

            result += '\t %s version %s, included from:\n' % (marked_for_install, version)
