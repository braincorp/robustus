...
def test_pereditable(tmpdir):
    ...
    # create some other git repos
    os.mkdir(repo_dir)
    for r in editable_sources:
        r_dir = os.path.join(repo_dir, r)
        os.mkdir(r_dir)
        os.system('cd "%s" && git init .' % r_dir)

    os.chdir(working_dir)
    robustus_executable = os.path.join(test_env, 'bin/robustus')
    test_requirements = os.path.join(working_dir, 'test_requirements1.txt')
    ...
    for r in editable_sources:
        assert os.path.exists(os.path.join(test_env, 'src', r, 'foo'))
